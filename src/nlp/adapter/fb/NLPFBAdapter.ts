...
import {DBOptions, FBAdapter, FBDatabase} from "graphql-sql-bridge";
...
export default class NLPFBAdapter extends FBAdapter {
    ...
    public async getTables(): Promise<INLPTable[]> {
        ...
        try {
            const nlpTables = await FBDatabase.executeDatabase<IFBTable[]>(<DBOptions>this.source,
                async (database: FBDatabase) => {
